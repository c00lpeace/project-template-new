...
class S3Service:
                ...
                with zipfile.ZipFile(io.BytesIO(zip_content), "r") as zip_ref:
                    ...
                    for root, _, files in os.walk(temp_path):
                        for file_name in files:
                            ...
                            logger.debug("압축 해제 파일 S3 업로드: s3_key=%s", s3_key)

            logger.info("ZIP 압축 해제 및 업로드 완료: %d개 파일", len(uploaded_files))
    ...
    def download_file(
        ...
        try:
            ...
            try:
                ...
                if isinstance(exc, ClientError) and hasattr(exc, "response"):
                    ...
                    if error_code == "NoSuchKey":
                        logger.error("S3 파일을 찾을 수 없습니다: s3_key=%s", s3_key)
            ...
            # 다른 예외는 그대로 전파
            logger.error("S3 파일 다운로드 실패: s3_key=%s, error=%s", s3_key, str(exc))
            raise
    ...
    def download_program_file(
        ...
        try:
            ...
            logger.error(
                "프로그램 파일 다운로드 실패: file_type=%s, " "program_id=%s, error=%s",
                file_type,
                program_id,
                str(e),
            )
    ...
    async def delete_files_by_prefix(self, prefix: str) -> int:
        ...
        try:
            ...
            logger.error("S3 파일 일괄 삭제 실패: prefix=%s, error=%s", prefix, str(e))
            raise
